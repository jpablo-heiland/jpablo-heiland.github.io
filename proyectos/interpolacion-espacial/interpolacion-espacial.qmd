...
El presente documento describe el flujo de trabajo implementado para la **Interpolación Espacial** de datos de Conductividad Eléctrica Aparente (CEa) del suelo. El conjunto de datos inicial consiste en un archivo vectorial de puntos relevado mediante un sensor *VERIS* a dos profundidades de muestreo (0–30 cm y 0–90 cm).
...
Tanto [gstat]{.func-tecnica} como [geoR]{.func-tecnica} son librerías de R diseñadas para el análisis geoestadístico, especialmente para:
...
Para leer datos se usó la función `st_read()` del paquete [sf]{.func-tecnica}. Esta función devuelve un objeto clase [Simple Feature (sf)]{.func-tecnica}.

Un objeto [sf]{.func-tecnica} es, esencialmente, un [data.frame]{.func-tecnica} (estructura de datos más común de R) que tiene una columna especial llamada [geometry]{.func-tecnica}, la cual almacena la representación espacial (puntos, líneas o polígonos) que corresponde a cada fila. Todas las librerías usadas en este documento son compatible con esta estructura de datos, por lo que no es necesario modificarla.
...
Se visualizan las primeras 5 filas de set de datos usando la librería [kableExtra]{.func-tecnica}:
...
Verificar la clase del objeto mediante `class()`:
...
Comprobar el Sistema de Referencia de Coordenadas (CRS) del objeto a través de `st_crs()`:

```{r}
st_crs(veris)
```

# Visualizar de los datos de CEa

En esta sección, se procede a la visualización espacial de los puntos de CEa mediante el paquete [ggplot2]{.func-tecnica}. Se realiza para examinar la densidad del muestreo, evaluar la cobertura espacial del sensor VERIS en el lote y observar a grandes rasgos la variabilidad del suelo para ambas profundidades (0-30 y 0-90 cm).
...
Se generan los mapas correspondientes a cada profundidad del suelo, accesibles a través de las siguientes pestañas
...
Se usó la función `variogram()` del paquete [gstat]{.func-tecnica}.
...
El semivariograma empírico muestra la relación espacial observada en los datos, pero al estar compuesto por puntos experimentales puede resultar irregular. Por esta razón, se ajusta una curva teórica que resume y representa esa tendencia general. De esta manera, se obtiene un modelo matemático continuo que describe cómo varía la CEa en el espacio y que puede ser utilizado por el método de Kriging.

En la Geoestadística, el ajuste asegura que el modelo sea insesgado y óptimo (MELI).

El ajuste se realizó mediante el algoritmo de Mínimos Cuadrados Ponderados (utilizando la función `fit.variogram()` de [gstat]{.func-tecnica}), el cual busca minimizar la desviación entre los puntos observados y la curva teórica. Se evaluaron tres modelos fundamentales por su relevancia en estudios agronómicos:

-   Modelo Esférico
-   Modelo Exponencial
-   Modelo Gaussiano

A través del ajuste, se determinaron los tres parámetros claves del semivariograma, que definen la morfología de la variabilidad (@fig-semivariograma):
...
A partir de la inspección visual de los semivariogramas, se podría inferir que el **Modelo Gaussiano** es el que mejor representa la variabilidad espacial de la CEa.
...
La interpolación espacial se ejecutó mediante el algoritmo de Kriging Ordinario, utilizando la función `krige()` del paquete [gstat]{.func-tecnica}. Esta función permite predecir valores en ubicaciones donde no se tomaron muestras, usando el modelo de variograma ajustado (@sec-ajust) y la información espacial de los puntos conocidos.

Para ejecutar la interpolación mediante la función `krige()`, el flujo de trabajo requiere convertir los objetos de la clase [sf]{.func-tecnica} a la clase [sp]{.func-tecnica}. Esta conversión es por cuestiones de compatibilidad: el paquete [gstat]{.func-tecnica} fue desarrollado sobre la arquitectura del paquete [sp]{.func-tecnica}.

El algoritmo de Kriging requiere dos elementos espaciales: los puntos observados y una grilla vacía sobre la cual calculará las estimaciones. Para que `krige()` funcione correctamente tanto los puntos como la grilla deben estar en clases espaciales compatibles del paquete [sp]{.func-tecnica}.

## Realizar el Kriging

En primer lugar, se convierte el objeto [sf]{.func-tecnica} a [sp]{.func-tecnica}:
...
Se le asigna el mismo CRS que el del objeto [veris_sp]{.func-tecnica}:
...
El objeto resultante ([krig_res]{.func-tecnica}) no constituye una superficie continua en primera instancia, sino que consiste en una cantidad enorme de puntos espaciados de forma muy densa, cuyos centroides corresponden exactamente a los nodos de la grilla. Para cada nodo espacial coordenado $(X,Y)$, el objeto [krig_res]{.func-tecnica} calcula y almacena internamente dos atributos fundamentales:

-   Predicción del Atributo ([var1.pred]{.func-tecnica`): Es el valor estimado de la variable para ese punto del lote.

-   Varianza del Kriging ([var1.var]{.func-tecnica}): Es una medida cuantitativa del error cuadrático medio de la predicción en cada punto.

# Graficar

Para graficar los resultados se llevaron a cabo dos alternativas:

-   Alternativa A: **Representación Vectorial**, convirtiendo el resultado del Kriging a objeto [sf]{.func-tecnica}.
-   Alternativa B: **Representación Matricial Continua**: convirtiendo el resultado a [SpatRaster]{.func-tecnica}.

## Alternativa A

Una vez obtenidos los resultados de la interpolación espacial mediante el algoritmo de Kriging Ordinario (@sec-krig), el resultado se estructuró en formato vectorial como un objeto de la clase [sf]{.func-tecnica}:
...
Se crearon los cuantiles, la variable continua de predicción ([var1.pred]{.func-tecnica}) se transformó en una variable categórica ordinal de cuatro clases ([var1_q]{.func-tecnica}):
...
En este opción se migró del modelo vectorial al modelo de datos ráster. Para ello, los puntos cortados se convirtieron a la clase [SpatRaster]{.func-tecnica}.

> [SpatRaster]{.func-tecnica} es la clase central del paquete [terra]{.func-tecnica}. Este paquete está diseñado para manejar datos **ráster** de forma eficiente. A diferencia de un objeto vectorial ([sf]{.func-tecnica}), un ráster no guarda coordenadas individuales para cada dato. Un [SpatRaster]{.func-tecnica} representa el espacio como una grilla regular de celdas (píxeles) dispuestas en filas y columnas. Este objeto almacena cuatro componentes: (a) matriz de datos (filas y columnas), (b) la extensión espacial (Bounding Box), (c) el CRS, (d) las capas (un solo objeto SpatRaster puede contener una o múltiples capas).

El flujo de trabajo fue el siguiente:

-   **Convertir a clase SpatRaster**:

Se procedió a la conversión de la capa vectorial recortada hacia la clase [SpatRaster]{.func-tecnica} del paquete [terra]{.func-tecnica}, mediante la función `vect()`:
...
Convertir a objeto [sf]{.func-tecnica}:
...
Convertir el objeto sf a [SpatVector]{.func-tecnica}:
